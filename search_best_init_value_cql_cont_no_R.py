...
base_dir = "./d3rlpy_logs/fqe_./d3rlpy_logs/cql_training_continuous_no_R./configs/cql_cont_generated"
        ...
        if os.path.isdir(subsubdir_path):
            ...
            if os.path.exists(init_value_path) and os.path.exists(loss_path):
                try:
                    ...
                    filtered_df = merged_df[merged_df["Loss"] < 0.25].copy()
                    # Filter rows where Init Value < 0
                    filtered_df = filtered_df[filtered_df["Init Value"] < -0].copy()
                    
                    # Check if any rows match the criteria
                    if not filtered_df.empty and (merged_df["Init Value"] < 0).all():
...
if results:
    ...
    top_10.to_csv("cql_training_continuous_no_R_top_10_results.csv", index=False)
