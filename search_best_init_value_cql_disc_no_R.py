...
base_dir = "./d3rlpy_logs/fqe_./d3rlpy_logs/cql_training_discrete_no_R./configs/cql_disc_generated"
        ...
        if os.path.isdir(subsubdir_path):
            ...
            if os.path.exists(init_value_path) and os.path.exists(loss_path):
                try:
                    ...
                    filtered_df = merged_df[merged_df["Loss"] < 0.03].copy()
                    ...
                    if not filtered_df.empty and (merged_df["Init Value"] < -0.02).all():
...
if results:
    ...
    top_10.to_csv("cql_training_discrete_no_R_top_10_results.csv", index=False)
