...
CONFIG_DIR="./configs/bcq_disc_generated"
# Generate the correct config file name
CONFIG_FILE="${CONFIG_DIR}/bcq_config_${SLURM_ARRAY_TASK_ID}.yaml"

python train.py --config "$CONFIG_FILE" --algorithm bcq --data_type discrete_no_R --data_path './Discrete Data/train_data_discrete_no_R_for_Survival.csv'
